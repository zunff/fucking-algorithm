...
public class LongestPalindromeSubseq516 {
    ...
    public static void main(String[] args) {
        String s = "bbbab";
        System.out.println(new LongestPalindromeSubseq516().longestPalindromeSubseqThird(s));
    }


    /**
     * dp[i][j] 表示 s[i...j]的最长回文字串的长度
     * {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            for (int j = i + 1; j < n; j++) {
                if (s.charAt(i) == s.charAt(j)) {
                    dp[i][j] = i + 1 < n && j - 1 >= 0 ? dp[i + 1][j - 1] + 2 : 0;
                } else {
                    dp[i][j] = Math.max(j - 1 >= 0 ? dp[i][j - 1] : 0, i + 1 < n ? dp[i + 1][j] : 0);
                }
            }
        }
        return dp[0][n - 1];
