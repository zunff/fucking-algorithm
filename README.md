...
> 核心思想：每步选择当前最优，期望通过局部最优达到全局最优。

### 1. 贪心 vs DP 怎么判断？

拿到一道题，问自己一个核心问题：

> **"做了当前最优选择后，剩下的子问题还能独立地找到最优解吗？"**

- **能** → 贪心选择性质成立 → 用贪心
- **不能**（当前选择会堵死后续路径）→ 需要 DP 记录所有状态

| | 贪心 | DP |
|---|------|-----|
| 路径 | 一条直线，不回头 | 一棵树，展开所有可能 |
| 核心条件 | 当前选择不污染后续子问题 | 有重叠子问题 + 最优子结构 |

**正例**：455 分发饼干 — 用最小够用的饼干，不影响后面分配（贪心 ✅）
**反例**：322 零钱兑换 — 面额 [1,3,4] 凑 6，贪心拿 4+1+1=3 枚，实际 3+3=2 枚。当前拿了大的，堵死了后续路径（需要 DP）

### 2. 贪心常见模型

| 模型 | 代表题 | 核心贪心策略 |
|------|--------|-------------|
| 排序 + 双指针 | 455 分发饼干 | 小饼干满足小胃口，保留大饼干 |
| 模拟 + 保留灵活资源 | 860 柠檬水找零 | 优先用 10+5 找零，保留万能的 5 元 |
| 累加局部收益 | 122 买卖股票 II | 每天独立，涨了就赚 |
| 两遍遍历 | 135 分发糖果 | 左右各扫一遍，取 max |
| 区间右端点排序 | 435 无重叠区间 | 选结束最早的，留给后面空间最大 |
| 维护最远边界 | 55 跳跃游戏 | 每步更新能到达的最远位置 |
| 反悔贪心 + 堆 | 630 课程表 III | 超时删最长课程，释放最多时间 |
